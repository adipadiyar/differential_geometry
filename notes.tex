...
\begin{definition}
    Define $\textbf{grad} \in L(C^\infty(M),\mathcal{X}(M))$ by $\langle v,\text{grad}_f(p) \rangle = Df_p(v)$ for all $v \in T_pM$
\end{definition}

\begin{definition}
    Define $\textbf{Hess}_p \in L(C^\infty(M),T_pM^{(2)})$ by \[f \to ((v,w) \to \langle \nabla_vX_f,w\rangle)\]
\end{definition}
...
\begin{definition}
    Suppose $M$ is connected. Then we define the \textbf{Riemannian metric} by \[d_M(p,q)=\text{inf}\{L(\gamma):\gamma \text{ is a piecewise smooth curve from }p \text{ to }q\}\]
\end{definition}
