...
\begin{definition}
    Define $\text{grad} \in L(C^\infty(M),\mathcal{X}(M))$ by $\langle v,\text{grad}_f(p) \rangle = Df_p(v)$ for all $v \in T_pM$
\end{definition}

\begin{definition}
    Define $\text{Hess}_p \in L(C^\infty(M),T_pM^{(2)})$ by \[f \to ((v,w) \to \langle \nabla_vX_f,w\rangle)\]
\end{definition}
...
\begin{definition}
    The curvature tensor of $X,Y,Z$ is defined by $R(X,Y,Z)=\nabla_X(\nabla_YZ)-\nabla_Y(\nabla_XZ)-\nabla_{[X,Y]}Z$
\end{definition}
...
\begin{definition}
    A vector field $V$ along $\gamma$ is called parallel if $V \in \text{ker} (\text{cov}_\gamma)$
\end{definition}
...
\begin{definition}
    Let $t_1,t_2 \in I$. We have the parallel transport map $P_{t_1t_2} \in L(T_{\gamma(t_1)}M,T_{\gamma(t_2)}M)$ 
\end{definition}
